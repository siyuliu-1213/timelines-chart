...
var timelines = Kapsule({
  ...
  init: function init(el, state) {
    var elem = select(el).attr('class', 'timelines-chart'); // state.svg = elem.append('svg');
    ...
    function renderTimelines(maxElems) {
      ...
      while (index < dataVertical.length) {
        ...
        var newItem = {
          ...
          labelVal: 'vertical' + ',' + min([1, cnt / 1000]) + ',' + cnt,
      ...
      var newSegments_rect = timelines_rect.enter().append('rect').attr('class', 'series-segment').attr('rx', 1).attr('ry', 1).attr('x', state.graphW / 2).attr('y', state.graphH / 2).attr('width', 0).attr('height', 0).style('fill', function (d) {
        ...
        select('.segment-tooltip').transition().duration(1000).style("opacity", "0").on("end", state.segmentTooltip.hide);
      });
      ...
      select(".segment-tooltip").on('mouseover', function () {
        select('.segment-tooltip').transition().style("opacity", "1"); // state.segmentTooltip.style("opacity", "1");
      }).on('mouseout', function () {
        select(".segment-tooltip").transition().duration(1000).style("opacity", "0").on("end", state.segmentTooltip.hide);
      ...
      newSegments_rect.on('mouseover', function () {
        ...
        moveToFront()(this); // const hoverEnlarge = state.lineHeight*hoverEnlargeRatio;
        // original code for rendering only rect
      ...
      newSegments_circle.on('mouseover', function () {
        ...
        select(this).transition().duration(70).attr('cx', function (d) {
          return state.xScale(d.timeRange[0]);
        }).attr('cy', function (d) {
        ...
        // code for rendering circle
        select(this).transition().duration(250).attr('cx', function (d) {
          return state.xScale(d.timeRange[0]);
        }).attr('cy', function (d) {
          return state.yScale(d.group + '+&+' + d.label);
