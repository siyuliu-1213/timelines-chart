...
var TimeOverview = Kapsule({
  ...
  init: function init(el, state) {
    ...
    state.brush = d3Brush.brushX().handleSize(24).on('end', function () {
...
var timelines = Kapsule({
  ...
  init: function init(el, state) {
    var elem = d3Selection.select(el).attr('class', 'timelines-chart'); // state.svg = elem.append('svg');
    // set svg to be responsive
    // zoom and pan

    state.svg = elem.append('svg'); // .call(d3Zoom().on("wheel.zoom", function(){
    ...
    state.overviewAreaElem = elem.append('div'); // .attr('width', '100%')
    // .attr('height','100%');
    ...
    function addZoomSelection() {
      state.graph.on('mousedown', function () {
        ...
        d3Selection.select(window).on('mousemove.zoomRect', function () {
          ...
          rect.attr('x', Math.min(startCoords[0], newCoords[0])).attr('y', 0) // .attr('y', Math.min(startCoords[1], newCoords[1]))
          .attr('width', Math.abs(newCoords[0] - startCoords[0])).attr('height', state.graphH); // .attr('height', Math.abs(newCoords[1] - startCoords[1]));
  ...
  update: function update(state) {
    ...
    function renderTimelines(maxElems) {
      ...
      var dataFilter_rect = function dataFilter_rect(d, i) {
        var info = d.labelVal.split(',');
        return info[0] == 'rect';
      };

      var dataFilter_circle = function dataFilter_circle(d, i) {
        var info = d.labelVal.split(',');
      ...
      var period = state.xScale.domain()[1].getTime() - state.xScale.domain()[0].getTime(); // console.log(state.xScale.domain()[0].getTime());
      // console.log(state.xScale.domain()[1].getTime());
      // console.log(state.graphW);

      var onePixelNms = period / state.graphW; // console.log(onePixelNms);

      var dataFilter_vertical = function dataFilter_vertical(d, i) {
        var info = d.labelVal.split(',');
        return info[0] == 'vertical';
      }; // This data contains only those in selected timerange


      var dataVertical = state.flatData.filter(dataFilter).filter(dataFilter_vertical); // console.log(dataVertical);
      ...
      while (index < dataVertical.length) {
        var cnt = parseInt(dataVertical[pivot].labelVal.split(',')[2]);
        ...
        var newItem = {
          ...
          labelVal: 'vertical' + ',' + d3Array.min([1, cnt / 1000]) + ',' + cnt,
          timeRange: [dataVertical[pivot].timeRange[0], dataVertical[pivot].timeRange[1]],
      ...
      if (pivot < dataVertical.length) newDataVertical.push(dataVertical[pivot]); // console.log(newDataVertical[0]);
      // console.log(newDataVertical.length);

      state.lineHeight = state.graphH / state.nLines * 0.8; // // original code for rendering only rect
      ...
      var timelines_rect = state.graph.selectAll('rect.series-segment').data(state.flatData.filter(dataFilter).filter(dataFilter_rect), function (d) {
        return d.group + d.label + d.timeRange[0];
      });
      var timelines_circle = state.graph.selectAll('circle.series-segment').data(state.flatData.filter(dataFilter).filter(dataFilter_circle), function (d) {
        return d.group + d.label + d.timeRange[0];
      ...
      var timelines_vertical = state.graph.selectAll('rect.series-segment-vertical').data(newDataVertical, function (d) {
        return d.group + d.label + d.timeRange[0];
      ...
      timelines_rect.exit().transition().duration(state.transDuration).style('fill-opacity', 0).remove();
      timelines_circle.exit().transition().duration(state.transDuration).style('fill-opacity', 0).remove();
      timelines_vertical.exit().transition().duration(state.transDuration).style('fill-opacity', 0).remove();
      ...
      var newSegments_rect = timelines_rect.enter().append('rect').attr('class', 'series-segment').attr('rx', 1).attr('ry', 1).attr('x', state.graphW / 2).attr('y', state.graphH / 2).attr('width', 0).attr('height', 0).style('fill', function (d) {
        return state.zColorScale(d.val);
      }).style('fill-opacity', 0).on('mouseover.groupTooltip', state.groupTooltip.show).on('mouseout.groupTooltip', state.groupTooltip.hide).on('mouseover.lineTooltip', state.lineTooltip.show).on('mouseout.lineTooltip', state.lineTooltip.hide).on('mouseover.segmentTooltip', state.segmentTooltip.show).on('mouseout.segmentTooltip', function (d) {
        d3Selection.select('.segment-tooltip').transition().duration(1000).style("opacity", "0").on("end", state.segmentTooltip.hide);
      });
      var newSegments_circle = timelines_circle.enter().append('circle').attr('class', 'series-segment').attr('cx', state.graphW / 2).attr('cy', state.graphH / 2).attr('r', 0).style('fill', function (d) {
        return state.zColorScale(d.val);
      }).style('fill-opacity', 0).on('mouseover.groupTooltip', state.groupTooltip.show).on('mouseout.groupTooltip', state.groupTooltip.hide).on('mouseover.lineTooltip', state.lineTooltip.show).on('mouseout.lineTooltip', state.lineTooltip.hide).on('mouseover.segmentTooltip', state.segmentTooltip.show).on('mouseout.segmentTooltip', function (d) {
        d3Selection.select('.segment-tooltip').transition().duration(1000).style("opacity", "0").on("end", state.segmentTooltip.hide);
      });
      var newSegments_vertical = timelines_vertical.enter().append('rect').attr('class', 'series-segment-vertical').attr('rx', 1).attr('ry', 1).attr('x', state.graphW / 2).attr('y', state.graphH / 2).attr('width', 0).attr('height', 0).style('fill', function (d) {
        return state.zColorScale(d.val);
      }).style('fill-opacity', 0).on('mouseover.groupTooltip', state.groupTooltip.show).on('mouseout.groupTooltip', state.groupTooltip.hide).on('mouseover.lineTooltip', state.lineTooltip.show).on('mouseout.lineTooltip', state.lineTooltip.hide).on('mouseover.segmentTooltip', state.segmentTooltip.show).on('mouseout.segmentTooltip', function (d) {
        d3Selection.select('.segment-tooltip').transition().duration(1000).style("opacity", "0").on("end", state.segmentTooltip.hide);
      }); // add hover effect for segmentTooltip

      d3Selection.select(".segment-tooltip").on('mouseover', function () {
        d3Selection.select('.segment-tooltip').transition().style("opacity", "1"); // state.segmentTooltip.style("opacity", "1");
      }).on('mouseout', function () {
        d3Selection.select(".segment-tooltip").transition().duration(1000).style("opacity", "0").on("end", state.segmentTooltip.hide);
      ...
      newSegments_rect.on('mouseover', function () {
        if ('disableHover' in state && state.disableHover) return;
        svgUtils.moveToFront()(this); // const hoverEnlarge = state.lineHeight*hoverEnlargeRatio;
        // original code for rendering only rect
      ...
      newSegments_circle.on('mouseover', function () {
        if ('disableHover' in state && state.disableHover) return;
        svgUtils.moveToFront()(this); // const hoverEnlarge = state.lineHeight*hoverEnlargeRatio;
        // code for rendering circle

        d3Selection.select(this).transition().duration(70).attr('cx', function (d) {
          return state.xScale(d.timeRange[0]);
        }).attr('cy', function (d) {
          return state.yScale(d.group + '+&+' + d.label);
        }).attr('r', (state.lineHeight + hoverEnlarge) / 5).style('fill-opacity', 1);
      }).on('mouseout', function () {
        // code for rendering circle
        d3Selection.select(this).transition().duration(250).attr('cx', function (d) {
          return state.xScale(d.timeRange[0]);
        }).attr('cy', function (d) {
          return state.yScale(d.group + '+&+' + d.label);
        }).attr('r', state.lineHeight / 5).style('fill-opacity', .8);
      }).on('click', function (s) {
        if (state.onSegmentClick) state.onSegmentClick(s);
      });
      newSegments_vertical.on('mouseover', function () {
        if ('disableHover' in state && state.disableHover) return;
        svgUtils.moveToFront()(this); // const hoverEnlarge = state.lineHeight*hoverEnlargeRatio;
        // original code for rendering only rect
        ...
        d3Selection.select(this).transition().duration(70).attr('x', function (d) {
          return state.xScale(d.timeRange[0]) - hoverEnlarge / 2;
        }).attr('width', 1 + hoverEnlarge) // .attr('width', function (d) {
        //   return d3Max([1, state.xScale(d.timeRange[1])-state.xScale(d.timeRange[0])])+hoverEnlarge;
        // })
        .attr('y', function (d) {
          var ratio = d.labelVal.split(',');
        ...
        d3Selection.select(this).transition().duration(250).attr('x', function (d) {
          return state.xScale(d.timeRange[0]);
        }).attr('width', 1) // .attr('width', function (d) {
      ...
      timelines_vertical = timelines_vertical.merge(newSegments_vertical); // // original code for rendering only rect
      ...
      timelines_rect.transition().duration(state.transDuration).attr('x', function (d) {
        return state.xScale(d.timeRange[0]);
      }).attr('width', function (d) {
        return d3Array.max([1, state.xScale(d.timeRange[1]) - state.xScale(d.timeRange[0])]);
      }).attr('y', function (d) {
        return state.yScale(d.group + '+&+' + d.label) - state.lineHeight / 2;
      }).attr('height', state.lineHeight).style('fill-opacity', .8); // code for rendering circle

      timelines_circle.transition().duration(state.transDuration).attr('cx', function (d) {
        return state.xScale(d.timeRange[0]);
      }).attr('cy', function (d) {
        return state.yScale(d.group + '+&+' + d.label);
      }).attr('r', state.lineHeight / 5).style('fill-opacity', .8);
      timelines_vertical.transition().duration(state.transDuration).attr('x', function (d) {
        return state.xScale(d.timeRange[0]);
      }).attr('width', 1) // .attr('width', function (d, i) {
      //   return d3Max([1, state.xScale(d.timeRange[1])-state.xScale(d.timeRange[0])]);
      // })
      .attr('y', function (d) {
        var ratio = d.labelVal.split(',');
